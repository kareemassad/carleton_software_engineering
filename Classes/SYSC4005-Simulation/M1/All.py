...
class Inspector2:
    def __init__(self, env, name, service_time, B3: Buffer, B5: Buffer) -> None:
        self.env = env
        self.name = name
        self.service_time = service_time
        self.B3 = B3
    ...
    def inspect(self, component):
        yield self.env.timeout(component.service_time)
        print(f"{self.name} inspected {component.name} at {self.env.now:.2f}")
        # self.buffer.put(component)
